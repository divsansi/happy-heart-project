...
<SignedIn let:user>
	...
	<main>
		...
		<section class="flex flex-col" style="justify-content: center; gap: 20px;">
			<Collection
				ref={query(
					collection(db, `/counselor/${user.uid}/chats`),
					where('chatuid', '==', data.props.id),
					orderBy('createdAt', 'asc'),
					limit(40)
				)}
				let:data let:count>
				<a class="flex flex-row" style="color: white; width: fit-content; align-items: center; gap: 5px;" href="/">
